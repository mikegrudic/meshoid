...
def planck_function(freqs, T):
    ...
    -------
    I: array_like
        Intensity of blackbody in erg/s/cm^2/sr
    """
...
def planck_function_dT(freqs, T):
    ...
    u(f,T) = dB(f,T)/dT = 2 h^2 f^4/(c^2 k_B T^2) exp(h f/(k_B T)) / (exp(h f/(k_B T) -1 ))^2
    ...
    -------
    u: array_like
        Temp derivative of blackbody function in erg/s/cm^2/sr/K
    """
    h_over_kB, h, c = 4.799243073366221e-11, 6.62607015e-27, 29979245800.0
    u = 2 * h * h_over_kB * freqs**4 / (c**2 * T**2) / np.cosh(freqs * h_over_kB / T) ** 2 / 4
    return u
